<template>
    ...
    <client-layout title = "Setting" :user = "user">
            ...
            <div class="py-4 px-6">
                <form @submit.prevent="updateUser(user.id)">
                    <div class="flex flex-col">
                        <div class="flex justify-center">
                          <div class ="h-40 w-40 overflow-hidden px-auto bg-gray-600  rounded-full border-4 border-gray-600">
                              <img class="h-32 w-40" :src="imageData ? imageData : user.avatar ? user.avatar:'/images/noImage.png'" alt="">
                    ...
                    <div class="mt-4 w-full">
                        <input type="text" name="name" placeholder="Full name" v-model="user.name"
                    ...
                    <div class="mt-4 w-full">
                        ...
                        <input disabled :type="type" name="password" placeholder="Password" :value="user.user_password"
                        ...
                        <div class="flex divide-x divide-gray-400">
                          ...
                          <a href = ""  v-if="type == 'text'"
                            v-clipboard:copy="this.type == 'password' ? '**********':user.user_password"
                            v-clipboard:success="onCopy"
                            v-clipboard:error="onError"
                            class="text-xs text-blue-300 mt-2 h-10 bg-gray-300 px-1 rounded-r pt-2"> Copy </a>
                        </div>
                        </div>
                         <input v-if="changePassord" type="password" name="password" placeholder="New password" v-model="user.password"
                               class="w-full mt-2 text-sm py-2 px-4 bg-gray-100 text-gray-700 border border-gray-300 rounded  block appearance-none placeholder-gray-500 focus:outline-none focus:bg-white"/>
                    </div>
...
<script>
    ...
    export default {
        ...
        props: ['tags' , 'errors','successMessage'],
