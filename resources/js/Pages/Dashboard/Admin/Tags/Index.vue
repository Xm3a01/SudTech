<template>
  <Layout title="Dashboard . tags" :user="user">
    ...
    <div class="bg-gray-100 p-6 rounded shadow">
        ...
        <div class="shadow flex  rounded">
            ...
            <div>
              <inertia-link href="tags/create"  class="px-3 py-1 font-medium shadow  bg-blue-500 hover:bg-blue-400 text-white text-sm rounded"> + New</inertia-link>
            </div>
                    ...
                    <div
                        ...
                        <table class="min-w-full shadow-lg">
                            ...
                            <!-- <tr v-for ="job in paginateJob.data ? paginateJob.data : jobs.data" :key="job.id"> -->
                                <tr v-for="tag in filters" :key="tag.id">
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <div class="flex items-center">
                                        <!-- <div class="flex-shrink-0 h-10 w-10">
                                        </div> -->
                                        <div class="mx-2">
                                          <div class="text-sm leading-5 font-medium text-gray-600"> {{tag.name}}</div>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                    <inertia-link :href="'/dashboard/tags/'+tag.id+'/edit'" class="px-2 py-1 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-medium rounded">Edit</inertia-link>
                                    <inertia-link href ='' @click.prevent="deletejob(tag.id)" class="px-2 py-1 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded">Delete</inertia-link>
                                </td>
                            </tr>
                            </tbody>
                        </table>
...
props:['user','tags','successMessage'],
...
methods: {
    getTags(page = 1) {
        axios.get('tag/paginate?page='+ page)
          .then((res) => {
              this.all = res.data;
          })
    },
